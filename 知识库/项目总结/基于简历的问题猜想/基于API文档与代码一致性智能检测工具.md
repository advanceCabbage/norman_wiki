...
- Continue 的 CodeBase 分为**文件索引创建（项目初始阶段）和文件检索**两个阶段
	- **文件索引创建阶段**包含分块索引、分段索引、全文索引、向量索引：<font color="#c0504d">TODO 查看源码</font>
		- 分块索引：
		- 分段索引：
		- 全文索引：
		- 向量索引：
	- **文件检索**：
		- 并行使用多种检索策略完成项目级检索述求，并行检索完成后对检索内容进行重排序处理，LLM 创建 repoMap，基于语义进行项目文件搜索<font color="#c0504d">TODO 查看源码</font>
- Serena 无任何前置操作，
