## 一、RPC 通用概念
**RPC = Remote Procedure Call（远程过程调用**）——让一个程序像调用本地函数一样去调用另一个进程/机器上的函数，中间的序列化、传输、反序列化对调用方透明。常见实现有 gRPC、JSON-RPC、Thrift 等。**RPC 仅仅是约定的协议，无实际的约束效果，需要达成共识遵守**
