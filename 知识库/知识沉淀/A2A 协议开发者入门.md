...
> 本文面向第一次接触 A2A 的开发者。示例基于 A2A **v1.0** 的规范；具体字段与 SDK 以权威的 [`a2a.proto`](https://github.com/a2aproject/A2A/blob/main/specification/a2a.proto) 为准。
...
A2A 最初由 Google 开发，后捐赠给 Linux Foundation；当前由来自 Google、IBM Research、Microsoft、AWS、SAP、Salesforce、ServiceNow、Cisco 等组织参与的技术治理机制维护。最新发布规范为 v1.0。[
A2A 官方介绍](https://a2a-protocol.org/latest/) [规范版本页](https://a2a-protocol.org/latest/specification/)
...
A2A 不替代 MCP，也不规定 Agent 内部的子 Agent 如何工作；这两件事分别应使用 MCP 或你选择的 Agent 框架。[A2A 与 MCP 的官方说明](https://a2a-protocol.org/latest/)
...
- **独立服务边界**：专业 Agent 是独立进程、容器、服务或团队所有的模块。
- **异构生态**：调用方和被调用方可能使用不同模型、语言、框架或供应商。
- **任务不是一次函数调用**：任务可能持续数秒至数天，需要进度、取消、断线恢复、补充信息或人工审批。
- **能力需要被发现和替换**：希望编排器根据 Agent Card 选择合适 Agent，或未来替换某个 Agent 而不改上游接口。
- **跨信任边界**：跨部门、跨租户、跨公司，需要清晰的认证、授权、审计和任务数据隔离。
- **产物丰富**：结果可能是 JSON、文件、图片、报告或分块流式内容，而非一小段文本。
...
`SendMessage` 对简单任务可以直接返回一条 `Message`；但复杂或长任务应该立即创建 `Task`，后续由查询、流式订阅或 webhook 获取结果。[
核心操作定义](https://github.com/a2aproject/A2A/blob/main/docs/specification.md)
...
**WebSocket 不是 v1 的标准核心绑定。**如果确有需要，可定义自定义 binding，并在 Agent Card 中用 URI 标识它；自定义 binding 仍必须保持 A2A 的数据模型、操作语义、错误映射、认证与版本规则。它只有在客户端同样实现该 binding 时才能互通。[
自定义绑定要求](https://github.com/a2aproject/A2A/blob/main/docs/specification.md)

---

## 4. 发现、鉴权与调用：一次 A2A 请求的生命周期
...
生产服务必须使用 HTTPS/TLS；服务端必须在每个操作上执行认证和授权，并确保 `GetTask`、`ListTasks`、取消和订阅等操作只暴露调用方被授权访问的 Task。[
安全要求](https://github.com/a2aproject/A2A/blob/main/docs/specification.md)

---

## 5. 完整示例：JSON-RPC 2.0 的合同审查 Agent
...
若 Client 可能离线数小时，可声明并实现 `pushNotifications`，让 Client 给指定 Task 注册 HTTPS webhook；收到通知后 Client 再调用 `GetTask` 拉取完整结果。[
流式和异步任务说明](https://github.com/a2aproject/A2A/blob/main/docs/topics/streaming-and-async.md)
...
- [`a2a.proto`：规范数据模型的权威来源](https://github.com/a2aproject/A2A/blob/main/specification/a2a.proto)
- [A2A 官方介绍](https://a2a-protocol.org/latest/)
- [A2A 与 MCP](https://github.com/a2aproject/A2A/blob/main/docs/topics/a2a-and-mcp.md)
- [流式与异步长任务](https://github.com/a2aproject/A2A/blob/main/docs/topics/streaming-and-async.md)
